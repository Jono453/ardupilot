        ...
        if (uart != nullptr) {
            ...
            switch (state[i].protocol) {
                ...
                case SerialProtocol_EFI_MS:
                    ...
                    uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
                    break;
                case SerialProtocol_EFI_Intelliject:
                    state[i].baud = AP_SERIALMANAGER_EFI_INTELLIJECT_BAUD;
                    uart->begin(map_baudrate(state[i].baud),
                    		AP_SERIALMANAGER_EFI_INTELLIJECT_BUFSIZE_RX,
							AP_SERIALMANAGER_EFI_INTELLIJECT_TX);
                    uart->set_flow_control(AP_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
